...
/**
 * @brief 3D_tensor, 4D_tensor를 입력으로 받아, 각각 2D_tensor로 변환한 뒤, 2D matrix multiplication을
 *        수행하는 함수. 2D matrix multiplication을 완료했을 때 나오는 결과는 항상 3D_tensor 여야 한다.
 *
 * @Usage: im2col(...);
 *
 * @param  convolution_2D와 stride_size를 제외하고는 동일한 input data를 갖는다. stride_size는 이번
 *         프로젝트에서는 무조건 1이라고 가정하기 때문입니다.
 * @return None. 별도의 return은 필요없습니다. 단지 함수 내부에서 text file로 결과를 출력해 주시면 됩니다.
 */
