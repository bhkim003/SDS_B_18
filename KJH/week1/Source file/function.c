...
uint8_t*** convolution_2D(uint8_t*** ifmap, uint8_t**** filter, int input_channel, 
    ...
    // Mathematical Expression for output tensor size
    ...
    // Consider padding in input tensor. 
    uint8_t*** _ifmap = NULL;
    if(padding_size > 0) {
        ...
        for (int i = 0; i < input_channel; i++) {
            for (int j = 0; j < Height; j++) {
                ...
                _ifmap[i][j + padding_size][k + padding_size] = ifmap[i][j][k];
            }
        }
    }
